...
func NewRouter(store *ServiceStore, envOrchestrator orchestrator.EnvironmentOrchestrator, argoLinks *orchestrator.ArgoLinks, repositories providers.RepositoryProvider, webhookAdapters map[scm.Provider]scm.WebhookAdapter, logger *zap.Logger) http.Handler {
	handlers := NewHandlers(store, envOrchestrator, argoLinks, repositories, webhookAdapters, logger)
