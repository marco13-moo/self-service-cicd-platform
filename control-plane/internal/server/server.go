...
func New(
	...
	if err != nil {
		...
		return nil, fmt.Errorf("initialize tenant authorization: %w", err)
	}
	if err = store.EnsureTenants(context.Background(), tenantAuthorizer.TenantIDs()); err != nil {
		if database != nil {
			_ = database.Close()
		}
	...
	return &Server{
		...
		reconciler: reconciler.NewSCMCommandReconciler(store, commandStore, envOrchestrator, cfg.Reconciler.PreviewTTL, reconciler.PreviewRuntimeConfig{
			...
			GitHubCloneBaseURL: cfg.GitHub.CloneBaseURL,
